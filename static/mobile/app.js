...
function openAddMenu(e, t) {
  ...
  const x = Math.min(Math.max(8, rect.left - 80), window.innerWidth - 240);
  const y = Math.min(rect.bottom + 4, window.innerHeight - 280);
  m.style.left = x + "px";
  m.style.top = y + "px";
...
$("#search-input").addEventListener("search", () => { if (!$("#search-input").value) return; doSearch(); });
